...
class Iki_Grid_Cell_Image {
	...
	protected function get_image( $img_id ) {
		...
		$classes          = Iki_Utils::sanitize_html_class_array( $classes );
		...
		if ( ! empty( $img_id ) ) {
			...
			if ( $this->lazy_load && ! empty( $img_src ) ) {
				...
				$thumbnail_image = $this->iki_canvas_thumb( $img_id, 'thumbnail', $img_src );
				$img_src         = '';
			} else {
				if ( ! empty( $img_src ) ) {
	...
	protected function calculate_image_size( Iki_Grid $grid = null ) {
		...
		if ( $current_cell->get_row_cells() == 1 && $asset_orientation == 'landscape' ) {
			...
			//third cell in portfolio orientation should be very very large
			//            $asset_size = ($current_cell->row_cells == 3
			//                && $current_cell->position_in_row === 3
			//                && $asset_orientation != 'portrait'
			//                && $asset_orientation != 'square') ? 1 : 2;
			$asset_size = 2;
