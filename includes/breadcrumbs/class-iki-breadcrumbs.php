...
class Iki_Breadcrumbs {
	...
	public function __construct( $settings = array() ) {
		...
		$this->settings['labels'] = array(
			'homepage-title' => __( 'Homepage', 'iki_toolkit' ),
			'blogpage-title' => __( 'Blog', 'iki_toolkit' ),
			'404-title'      => __( '404 Not found', 'iki_toolkit' ),
		);
	...
	private function build_breadcrumbs() {
		...
		if ( is_array( $custom_page ) && ! empty( $custom_page ) ) {
			$return[] = $custom_page;
			$return   = apply_filters( 'iki_toolkit_breadcrumbs_build', $return );
		...
		if ( is_404() ) {
			...
			$search['type'] = 'search';
			$search['name'] = __( 'Searching for:', 'iki_toolkit' ) . ' ' . get_search_query();
			$s              = '?s=' . apply_filters( 'iki_toolkit_breadcrumbs_search_query', get_search_query() );
		...
		foreach ( $return as $key => $item ) {
			if ( empty( $item['name'] ) ) {
				$return[ $key ]['name'] = __( 'No title', 'iki_toolkit' );
			}
		}
		...
		if( intval( $paged['name'] ) > 0 ){
			$paged['name'] = __('Page', 'iki_toolkit') . ' ' . $paged['name'];
			$return[] = $paged;
		}*/
