		...
		<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
			...
			<p class="field-custom description description-wide">
				<label for="edit-menu-item-iconclass-<?php echo $item_id; ?>">
					<?php esc_html_e( 'Font awesome css icon class', 'iki-toolkit' ); ?><br/>
				...
				<label for="edit-menu-item-type-menu-<?php echo $item_id; ?>">
					...
					<select id="edit-menu-item-type-menu-<?php echo $item_id; ?>"
						...
						<option
								value="m" <?php if ( esc_attr( $item->iki_menu_icon_size ) === "m" || esc_attr( $item->iki_menu_icon_size ) == '' ) {
							echo 'selected="selected"';
						} ?>><?php esc_html_e( 'Small', 'iki-toolkit' ) ?>
						</option>
						<option
								value="l" <?php if ( esc_attr( $item->iki_menu_icon_size ) === "l" ) {
							echo 'selected="selected"';
						} ?>><?php esc_html_e( 'Medium', 'iki-toolkit' ); ?>
						</option>
						<option
								value="xl" <?php if ( esc_attr( $item->iki_menu_icon_size ) === "xl" ) {
							echo 'selected="selected"';
						} ?>><?php esc_html_e( 'Large', 'iki-toolkit' ); ?>
						</option>
						<option
								value="xxl" <?php if ( esc_attr( $item->iki_menu_icon_size ) === "xxl" ) {
							echo 'selected="selected"';
						} ?>><?php esc_html_e( 'Extra large', 'iki-toolkit' ); ?>
						</option>
					</select>
				</label>
			</p>
			...
			if ( $depth === 0 ) { ?>
				...
				<p class="field-custom description ">
					<label for="edit-menu-item-type-menu-<?php echo $item_id; ?>">
						<?php esc_html_e( 'Float menu item to', 'iki-toolkit' ); ?><br/>
						<select id="edit-menu-item-type-menu-<?php echo $item_id; ?>"
				...
				<p class="field-custom description description-wide">
					<?php esc_html_e( 'Assign Z-index value (number - integer) to the button that has a stamp , so if the stamp is outside of the button , it won\'t be overlaped by some other top level button menu.', 'iki-toolkit' ); ?>
				</p>
				<p class="field-custom description description-wide">
					<?php esc_html_e( ' This is totally optional.', 'iki-toolkit' ); ?>
				</p>

				<p class="field-custom description description-wide1">
					<label for="edit-menu-item-stamp-z-<?php echo $item_id; ?>">
						<?php esc_html_e( 'Z Index:', 'iki-toolkit' ); ?><br/>
				...
				<?php if ( $this->iki_content_block_cpt_present ) { ?>
					<br>
					<div id="iki-cb-options-<?php echo $item_id; ?>" class="iki-cb-options">
						...
						<p class="field-custom description description-wide description-thin-custom">
							<label for="edit-menu-content-block-<?php echo $item_id; ?>">
								<?php esc_html_e( 'Menu Block', 'iki-toolkit' ); ?><br/>
								<select class="description-wide" id="edit-menu-content-block-<?php echo $item_id; ?>"
									...
									if ( ! empty( $this->block_posts ) ) { ?>
										<option
												value="false"><?php esc_html_e( 'Don\'t use content block', 'iki-toolkit' ); ?></option>
							...
							<label>
								<input type="radio" name="menu-item-iki_menu_cb_width[<?php echo $item_id; ?>]"
									<?php checked( $item->iki_menu_cb_width, 'full' ); ?>
									   value="full"/>
								<?php esc_html_e( 'Content block width equals menu width', 'iki-toolkit' ); ?>
							</label><br>
							<label>
								<input type="radio" name="menu-item-iki_menu_cb_width[<?php echo $item_id; ?>]"
									...
									checked( $item->iki_menu_cb_width, '' ); ?>
									   value="fixed"/>
								<?php esc_html_e( 'Content block width equals theme fixed width', 'iki-toolkit' ); ?>
							</label><br>
						</p>

						<p class="field-custom description ">
							<label>
								<?php esc_html_e( 'Menu block position', 'iki-toolkit' ); ?> <br>
			...
			<p class="field-custom iki-stamp-chb-parent description description-wide ">
				<label for="edit-menu-item-has-new-stamp-<?php echo $item_id; ?>">
					...
					<?php esc_html_e( 'Insert "New" stamp on the menu button ? (Needs to be checked for the stamp to be displayed)', 'iki-toolkit' ); ?>
				</label>
			</p>
			...
			<div id="iki-stamp-options-<?php echo $item_id; ?>" class="iki-stamp-options">
				...
				<p class="field-custom description description-wide">
					<?php esc_html_e( 'Please provide a value toghether with the unit (px,%)', 'iki-toolkit' ) ?></p>
				...
				<p class="field-custom description description-wide">
					<label for="edit-menu-item-stamp-pos-right-<?php echo $item_id; ?>">
						<?php esc_html_e( 'Right (optional) :', 'iki-toolkit' ); ?><br/>
						<input type="text" id="edit-menu-item-stamp-pos-right-<?php echo $item_id; ?>"
							   class=" code edit-menu-item-custom"
							   name="menu-item-iki_stamp_pos_right[<?php echo $item_id; ?>]"
							   value="<?php echo esc_attr( $item->iki_stamp_pos_right ); ?>"/>
					</label>
				</p>
				...
				<p class="field-custom description description-wide">
					<label for="edit-menu-item-stamp-rotation-<?php echo $item_id; ?>">
						<?php esc_html_e( 'Minimum width: Please provide unit toghether with the value. Can be left empty.', 'iki-toolkit' ); ?>
						<br/>
