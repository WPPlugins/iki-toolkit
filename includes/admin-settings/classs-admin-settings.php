...
class Iki_Admin_Settings {
	...
	protected function create_options_form( $active_tab ) {
		...
		if ( empty( $active_tab ) ) {
			$this->sections[0]->settings_fields();
			$this->sections[0]->do_settings_sections();
			...
			foreach ( $this->sections as $section ) {
				if ( $section->name == $active_tab ) {
					$section->settings_fields();
					$section->do_settings_sections();
				}
			}
		}
	...
	public function display_settings() {
		...
		<?php
	} // end iki_toolkit_settings_display
}
