...
class Iki_API_Keys_Admin_Section extends Iki_Admin_Options_Section {
	...
	public function add_section_scripts( $hook ) {
		if ( 'settings_page_iki_toolkit_options' == $hook ) {
			wp_enqueue_script( 'iki-toolkit-api-test', IKI_TOOLKIT_ROOT_URL . 'js/admin/admin-settings.min.js', array( 'jquery' ), false, true );
		}
	}
	...
	protected function setup_option_fields() {
		...
		add_settings_field(
			...
			$this->settings_sections_id,
			array(
				'service'  => 'dribbble_api_key',
				'btn_id'   => 'iki-test-dribbble-api',
				'btn_text' => __( 'Test Dribbble API key', 'iki-toolkit' )
			)
		);
		add_settings_field(
			'500px_api_key',
			...
			$this->settings_sections_id,
			array(
				'service'  => '500px_api_key',
				'btn_id'   => 'iki-test-five-api',
				'btn_text' => __( 'Test 500px API key', 'iki-toolkit' )
			)
		);
	}
